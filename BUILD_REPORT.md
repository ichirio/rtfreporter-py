...
Two production-style clinical tables built from real ADaM data, mirroring the R
package's `showcase-dm` / `showcase-ae` articles.

### What it covers

| Example | Table | Routes exercised |
|---------|-------|------------------|
| `examples/showcase_dm.py` | Table 14.1.1 demographics | pandas DataFrame **and** `great_tables` GT |
| `examples/showcase_ae.py` | Adverse events by SOC / preferred term | pandas DataFrame, group-aware pagination |

Data: `examples/data/{adsl,adae}.csv`, a column/row subset of
`pharmaverseadam` (Apache-2.0, CDISC pilot study), vendored so the examples run
with no R and no network. `examples/adam_synthetic.py` is a seeded synthetic
alternative with the same column contract.

### Validated against R

`tests/test_showcase.py` (26 tests) asserts the Python output against
`data-raw/R_reference_numbers.txt`, generated by running the equivalent
computation in R over the same data. Everything below **matches exactly**:

- arm denominators 86 / 96 / 72;
- age n, Mean (SD), Median, Min-Max per arm;
- age-group, sex and race counts, including the zero-subject Asian level, which
  is retained rather than dropped;
- any-AE counts 65 / 84 / 68;
- SOC-level distinct-subject counts (cardiac 12/14/14, gastrointestinal
  17/15/19, general disorders 21/51/36), asserted to differ from the sum of the
  preferred terms beneath them;
- exactly 32 preferred terms passing the 3% filter, and the PT ordering;
- rendered RTF well-formed, ASCII-safe, multi-page, with a `(Cont.)` marker.

### Port defects the showcase exposed (all fixed)

Building the showcase against the R reference surfaced three real gaps. Each
was fixed in the library rather than worked around in the example:

1. **`split="group_force"` never continued a group.** It split only a group
   that by itself exceeded `max_rows` — i.e. `group_safe` behaviour — so no
   `(Cont.)` marker was ever emitted. R's `.split_group_force()` cuts on every
   `max_rows`, applies widow/orphan control, and repeats the group header. Now
   ported faithfully; R and Python return identical pages for the same input
   (checked directly: page 1 `A|0, A|1, A|2`, page 2 `A (Cont.)|NA, A|3, A|4`).
2. **`group_by` was a `NotImplementedError` stub.** All four R modes are now
   implemented with R's detection order.
3. **`blank_rows="between_groups"` raised `ValueError`.** The R shorthand now
   works, including inside a combining list.

Two tests had encoded defects 1 and 2 as expected behaviour; both were
corrected against the R ground truth rather than left passing.

### Still not reproduced

- The R articles render the same table through six (DM) and five (AE)
  different table frameworks. Python has two routes (pandas, `great_tables`),
  because gtsummary / rtables / tern / tfrmt / Tplyr have no Python equivalent.
  This is a limitation of the ecosystem, not of the port.
- Word screenshots of the rendered output are not included.
